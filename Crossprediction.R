...
##################################################################
##  Train on multiome of male donor Test on scRNA   ##
##################################################################
...
orig_x <- read.csv(".../Data/Concat/Male_Donor/X_GC_PB.csv", row.names = 1, header = TRUE)
orig_y <- read.csv(".../Data/Concat/Male_Donor/Y_GC_PB.csv", row.names = 1, header = TRUE)
################
# Test Data #
################
## new_x and new_y are X and Y matrices that we want to test the SLIDE model on 
new_x <- read.csv(".../Data/Concat/scRNA_Male_Donor/X_GC_PB.csv", row.names = 1, header = TRUE)
new_y <- read.csv(".../Data/Concat/scRNA_Male_Donor/Y_GC_PB.csv", row.names = 1, header = TRUE)

################
# SLIDE Model #
################
# These necessary files are available in the best SLDIE model out folder
LFs <- readRDS(".../AllLatentFactors.rds")
orig_z <- read.csv(".../z_matrix.csv", row.names = 1, header = TRUE)
slide_res <- readRDS(".../SLIDE_LFs.rds")
########################################
A_mat = LFs$A
beta = LFs$beta
